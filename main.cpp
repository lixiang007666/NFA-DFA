...
void eclouse(char c,string &he,edge b[]) //DFS
{ 
int k; 
for(k=0;k<N;k++) 
{ 
...
cout<<"请输入NFA各边信息,分别为 起点 有向弧权值 终点 最后以#结束: "<<endl; 
for(i=0;i<MAXS;i++) 
{ 
   cin>>b[i].first; 
   if(b[i].first=="#") break; 
...
for(i=0;i<m;i++) 
   for(j=0;j<endnode.length();j++) 
    if(d[i].find(endnode[j])<d[i].length()&&ednode.find(md[i].ltab)) 
     ednode+=md[i].ltab; 
endnode=ednode; 
cout<<endl<<"化简DFA如下: "<<endl; 
outputfa(len,m,md);  
cout<<"其中终态结点为: "<<endnode<<endl; 
return 0;
}
